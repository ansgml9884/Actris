...
public class LoadDatabase {
	...
	CommandLineRunner initDatabase(RankingRepository rankingRepository, ReplayRepository replayRepository) {
		return args -> {
			log.info("Preloading " + rankingRepository.save(Ranking.builder().name("김용수").score(100).note("내가 바로 빛용수다").replay_id(1L).build()));
			log.info("Preloading " + rankingRepository.save(Ranking.builder().name("양희영").score(200).note("서른에 꿈이 생겼다..@").replay_id(2L).build()));
			log.info("Preloading " + rankingRepository.save(Ranking.builder().name("오나영").score(300).note("텐텐 ㄱㄱ").replay_id(3L).build()));
			log.info("Preloading " + rankingRepository.save(Ranking.builder().name("김문희").score(400).note("나문희").replay_id(4L).build()));
